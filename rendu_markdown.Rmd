...
Then we are gonna send the itineraries to the api and wait for an answer, then you can display the results
...
saveRDS(object = route_mb_creuse, file = "route_mb_creuse.rds")
route_mb_creuse <- readRDS(file = "route_mb_creuse.rds")
...
saveRDS(object = route_mb_lyon, file = "route_mb_lyon.rds")
route_mb_lyon <- readRDS(file = "route_mb_lyon.rds")

route_mb_idf <- list()
for (i in 1:length(coord_osrm_idf)) {
  route_mb_idf[[i]] <- mb_directions(
    origin = coord_osrm_idf[[i]][[1]],
    destination = coord_osrm_idf[[i]][[2]],
    profile = "driving",
    access_token = my_token
  )
}

saveRDS(object = route_mb_idf, file = "route_mb_idf.rds")
route_mb_idf <- readRDS(file = "route_mb_idf.rds")

r_mb_creuse <- bind_rows(route_mb_creuse)
qtm(r_mb_creuse)
r_mb_lyon <- bind_rows(route_mb_lyon)
qtm(r_mb_lyon)
r_mb_idf <- bind_rows(route_mb_idf)
qtm(r_mb_idf)
```

## Fifth API: Googlemaps

Everybody know google maps so its a reference for our analysis test.
...
saveRDS(object = route_maps_creuse, file = "route_maps_creuse.rds")
route_maps_creuse <- readRDS(file = "route_maps_creuse.rds")
...
saveRDS(object = route_maps_lyon, file = "route_maps_lyon.rds")
route_maps_lyon <- readRDS(file = "route_maps_lyon.rds")
...
for (i in 1:length(coord_osrm_idf)) {
  ...
  route_maps_idf[[i]] <- mp_get_routes(doc)
}
saveRDS(object = route_maps_idf, file = "route_maps_idf.rds")
route_maps_idf <- readRDS(file = "route_maps_idf.rds")
r_maps_idf<- bind_rows(route_maps_idf)
qtm(r_maps_idf)
```
...
for (i in 477:length(coord_osrm_creuse)) {
  route_ors_creuse[[i]] <- ors_directions(coord_osrm_creuse[[i]], format="geojson", output = "sf")
}
route_ors_lyon <- list()
for (i in 368:length(coord_osrm_lyon)) {
  route_ors_lyon[[i]] <- ors_directions(coord_osrm_lyon[[i]], format="geojson", output = "sf")
}
route_ors_idf <- list()
for (i in 481:length(coord_osrm_idf)) {
  route_ors_idf[[i]] <- ors_directions(coord_osrm_idf[[i]], format="geojson", output = "sf")
}

saveRDS(object = route_ors_creuse, file = "route_ors_creuse.rds")
saveRDS(object = route_ors_lyon, file = "route_ors_lyon.rds")
saveRDS(object = route_ors_idf, file = "route_ors_idf.rds")
route_ors_creuse <- readRDS(file = "route_ors_creuse.rds")
route_ors_lyon <- readRDS(file = "route_ors_lyon.rds")
route_ors_idf <- readRDS(file = "route_ors_idf.rds")
r_ors_idf <- bind_rows(route_ors_idf)
